...
export class App extends LitElement {
        ...
        return html`
            <div id="sidedrawer" class="mui--no-user-select">
                <div id="sidedrawer-brand" class="mui--appbar-line-height">
                    ...
                    <span class="mui--text-headline">ExpressLRS&trade;</span>
                </div>
            ...
            <header class="mui-appbar mui--appbar-height app-bg app-bar">
                <div>
                ...
                <div>
                    <div>${elrsState.settings?.product_name}</div>
                    <div>
    ...
    _generateSubMenu(item) {
        return Object.entries(item).map(([route, entry]) => {
            if (entry.when === undefined || entry.when()) {
                return html`
                    <li>
                        <a id="menu-${route}" href="#${route}" class="${route === this.currentRoute ? 'active' : ''}">${entry.icon}${entry.label}</a>
                    </li>
                `
            }
        })
    ...
    animateMainIn() {
        return new Promise(resolve => {
            ...
            this.mainEl.classList.add('route-fade')
            requestAnimationFrame(() => {
                this.mainEl.classList.remove('route-fade')
                requestAnimationFrame(() => {
                    this.mainEl.classList.remove('route-fade')
                    resolve()
                })
            ...
            return this.closeSidedrawer().then(() => this.ensureLoadedForRoute(route)).then(() => {
                return this.runWithSettingsRetry('Loading panel data...', async () => {
                    if (this.currentRoute) {
                        await this.waitForElementTransition(this.mainEl, () => this.mainEl.classList.add('route-fade'))
                    }
