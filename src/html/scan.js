...
function getPwmFormData() {
  ...
  while (inField = _(`pwm_${ch}_ch`)) {
    ...
    if (failsafe > 2135) failsafe = 2115; // Extended limits
    if (failsafe < 885) failsafe = 885;
    failsafeField.value = failsafe;
    let failsafeMode = failsafeModeField.value;

    const raw = (narrow << 21) | (mode << 17) | (invert << 16) | (inChannel << 12) | (failsafeMode << 22) | failsafe;
...
function updatePwmSettings(arPwm) {
  ...
  arPwm.forEach((item, index) => {
    const failsafe = (item.config & 4095); // 12 bits
    const failsafeMode = (item.config >> 22) & 3; // 2 bits
    const ch = (item.config >> 12) & 15; // 4 bits
    const inv = (item.config >> 16) & 1;
    const mode = (item.config >> 17) & 15; // 4 bits
    ...
    htmlFields.push(`<tr><td class="mui--text-center mui--text-title">${index + 1}</td>
            ...
            <td><div class="mui-textfield compact"><input id="pwm_${index}_limit_min" value="${item.limits.min}" size="6" class="pwmitm"/></div></td>
            <td><div class="mui-textfield compact"><input id="pwm_${index}_limit_max" value="${item.limits.max}" size="6" class="pwmitm"/></div></td>
            </tr>`);
    pinModes[index] = mode;
  });
  htmlFields.push('</table></div>');
