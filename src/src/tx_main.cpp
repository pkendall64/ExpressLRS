...
static void processDoublePayload(const uint8_t packageIndexProcessing, const uint8_t packageIndexOther, const uint8_t *payloadProcessing, const uint8_t * payloadOther, const uint8_t payloadSize, const bool tlmConfirm)
{
    if (config.GetAntennaMode() == TX_RADIO_MODE_GEMINI)
    {
        if (Radio.GetProcessingPacketRadio() == SX12XX_Radio_1)
        {
            packageIndexRadio1 = packageIndexProcessing;
            memcpy(tlmSenderDoubleBuffer, payloadProcessing, payloadSize);
            if (Radio.hasSecondRadioGotData)
            {
                packageIndexRadio2 = packageIndexOther;
                memcpy(tlmSenderDoubleBuffer + payloadSize, payloadOther, payloadSize);
            }
        }
        else
        {
            packageIndexRadio2 = packageIndexProcessing;
            memcpy(tlmSenderDoubleBuffer + payloadSize, payloadProcessing, payloadSize);
            if (Radio.hasSecondRadioGotData)
            {
                packageIndexRadio1 = packageIndexOther;
                memcpy(tlmSenderDoubleBuffer, payloadOther, payloadSize);
            }
        }
        ...
        if (packageIndexRadio1 == packageIndexRadio2 && packageIndexRadio1 != 0xFF)
        {
            ...
            TelemetryReceiver.ReceiveData(packageIndexRadio1, tlmSenderDoubleBuffer, 2 * payloadSize);
    ...
    {
        MspSender.ConfirmCurrentPayload(tlmConfirm);
        TelemetryReceiver.ReceiveData(packageIndexProcessing, payloadProcessing, payloadSize);
    }
}
...
bool ICACHE_RAM_ATTR ProcessTLMpacket(SX12xxDriverCommon::rx_status const status)
{
  ...
  if (OtaIsFullRes)
  {
    OTA_Packet8_s * const ota8 = &otaPktPtr->full;
    OTA_Packet8_s * const ota8Second = &otaPktPtrSecond->full;
    ...
    switch (otaPktPtr->std.type)
    {
      case PACKET_TYPE_LINKSTATS:
        ...
        processDoublePayload(ota8->tlm_dl.packageIndex, ota8Second->tlm_dl.packageIndex, ota8->tlm_dl.ul_link_stats.payload, ota8Second->tlm_dl.ul_link_stats.payload, sizeof(ota8->tlm_dl.ul_link_stats.payload), ota8->tlm_dl.tlmConfirm);
        break;
      ...
      case PACKET_TYPE_DATA:
        ...
        {
            processDoublePayload(ota8->tlm_dl.packageIndex, ota8Second->tlm_dl.packageIndex, ota8->tlm_dl.payload, ota8Second->tlm_dl.payload, sizeof(ota8->tlm_dl.payload), ota8->tlm_dl.tlmConfirm);
        }
        break;
    }
  }
  ...
  else
  {
    switch (otaPktPtr->std.type)
    {
      ...
      case PACKET_TYPE_DATA:
        ...
        {
            processDoublePayload(otaPktPtr->std.tlm_dl.packageIndex, otaPktPtrSecond->std.tlm_dl.packageIndex, otaPktPtr->std.tlm_dl.payload, otaPktPtrSecond->std.tlm_dl.payload, sizeof(otaPktPtr->std.tlm_dl.payload), otaPktPtr->std.tlm_dl.tlmConfirm);
        }
        break;
    }
  }
