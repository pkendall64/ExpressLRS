...
SerialTramp::SerialTramp(HardwareSerial &stream, const int8_t txPin)
...
void SerialTramp::queueMSPFrameTransmission(uint8_t *data)
{
    ...
    if (havePowerAndPitmode)
    {
        ...
        constexpr uint8_t maxPowerIndex = sizeof(powerLevelLUT) / sizeof(powerLevelLUT[0]) - 1;
        ...
        // Set power
        const uint16_t power = powerLevelLUT[powerIndex]; // Convert powerIndex to mW
