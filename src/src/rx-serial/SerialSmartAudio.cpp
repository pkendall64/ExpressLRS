...
void SerialSmartAudio::sendQueuedData(const uint32_t maxBytesToSend)
{
#if defined(PLATFORM_ESP32)
    ...
    {
        _fifo.lock();
        const uint8_t frameSize = _fifo.pop() - 1;
...
void SerialSmartAudio::forwardMessage(const crsf_header_t *message)
{
    ...
    const uint8_t innerLength = data[6];
    if (innerLength < 2)
    {
        return;
    }
    ...
    // Needed to avoid sequence-point compiler warning by assigning / using frameIndex
    // Set frequency
    ...
    uint8_t crcValue = crc.calc(tempFrame, frameIndex);
    ...
    const bool havePowerAndPitmode = innerLength >= 4;
    if (havePowerAndPitmode)
    {
        const uint8_t powerIndex = data[10];
        // Set power
        memset(tempFrame, 0, SMARTAUDIO_MAX_FRAME_SIZE);
        frameIndex = 0;
        tempFrame[frameIndex++] = SMARTAUDIO_HEADER_DUMMY;
