...
uint32_t SerialSBUS::sendRCFrame(bool frameAvailable, bool frameMissed, uint32_t *channelData)
{
    ...
    if ((!frameAvailable && !frameMissed && !effectivelyFailsafed) || _port->availableForWrite() < 25)
    {
    ...
    _port->write(0x0F);    // HEADER
    _port->write((byte *)&PackedRCdataOut, sizeof(PackedRCdataOut));
    _port->write((uint8_t)extraData);    // ch 17, 18, lost packet, failsafe
    _port->write((uint8_t)0x00);    // FOOTER
