...
SerialSUMD::SerialSUMD(HardwareSerial &stream, const int8_t txPin)
...
int32_t SerialSUMD::sendRCFrame(bool frameAvailable, bool frameMissed, uint32_t *channelData)
{
    ...
    const uint16_t crc = crc2Byte.calc(outBuffer, (SUMD_HEADER_SIZE + SUMD_DATA_SIZE_16CH), 0);
    outBuffer[35] = static_cast<uint8_t>(crc >> 8);
    outBuffer[36] = static_cast<uint8_t>(crc & 0x00ff);
