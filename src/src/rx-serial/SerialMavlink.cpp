...
int32_t SerialMavlink::sendRCFrame(const bool frameAvailable, bool frameMissed, uint32_t *channelData)
{
    if (!frameAvailable)
    {
...
void SerialMavlink::sendQueuedData(uint32_t maxBytesToSend)
{
    ...
    if ((now - lastSentFlowCtrl) > 10)
    {
        ...
        const mavlink_radio_status_t radio_status{
            .rxerrors = 0,
            .fixed = 0,
            .rssi = static_cast<uint8_t>(CRSF::LinkStatistics.uplink_Link_quality * 2.55),
            .remrssi = CRSF::LinkStatistics.uplink_RSSI_1,
            .txbuf = percentage_remaining,
            .noise = static_cast<uint8_t>(CRSF::LinkStatistics.uplink_SNR),
            .remnoise = 0,
        };
