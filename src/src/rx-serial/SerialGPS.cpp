...
int32_t parseDecimalToScaled(const char *str, const int32_t scale)
{
    char *end;
...
void SerialGPS::processSentence(uint8_t *sentence, const uint8_t size)
{
    if (size < 6)
    {
        return;
    ...
    if (sentence[3] == 'G' && sentence[4] == 'G' && sentence[5] == 'A')
    {
        ...
        if (ptr != nullptr)
        {
            const int32_t degrees = atoi(ptr) / 100;
            char minutes[20];
            strncpy(minutes, ptr + 2, 19);
            const int32_t minutesPart = parseDecimalToScaled(minutes, 10000000) / 60;

            gpsData.lat = degrees * 10000000 + minutesPart;
        }
        ptr = strchr(ptr, ',') + 1;

        if (ptr != nullptr && *ptr == 'S')
        {
...
void SerialGPS::sendTelemetryFrame() const
{
    ...
    crsfgps.p.gps_heading = htobe16(static_cast<uint16_t>(gpsData.heading));
