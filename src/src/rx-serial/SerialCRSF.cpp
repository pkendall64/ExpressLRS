...
void SerialCRSF::sendQueuedData(uint32_t maxBytesToSend)
{
    ...
    while (msp2crsf.FIFOout.size() > msp2crsf.FIFOout.peek() && (bytesWritten + msp2crsf.FIFOout.peek()) < maxBytesToSend)
    {
        ...
        noInterrupts();
        _stream->write(OutData, OutPktLen); // write the packet out
        interrupts();
...
int32_t SerialCRSF::sendRCFrame(bool frameAvailable, bool frameMissed, uint32_t *channelData)
{
    ...
    {
        const int32_t rssiDBM = CRSF::LinkStatistics.active_antenna == 0 ? -CRSF::LinkStatistics.uplink_RSSI_1 : -CRSF::LinkStatistics.uplink_RSSI_2;
